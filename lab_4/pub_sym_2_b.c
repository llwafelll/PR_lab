...
pthread_mutex_t mutex_k_pob, mutex_k_odd;
...
int main( void ){
  ...
  printf("\nLiczba kufli po zamknieciu baru: %d\n", k_odd - k_pob);
}
...
void * watek_klient (void * arg_wsk){
  ...
  for(i_wypite=0; i_wypite<ile_musze_wypic; i_wypite++){
    ...
    while (!(k_odd - k_pob > 0)) { /*busy waiting...*/ }

    pthread_mutex_lock(&mutex_k_pob);
    printf("\nKlient %d, wybrałem kufel %d\n", moj_id, kufel = k_odd - ++k_pob);
    pthread_mutex_unlock(&mutex_k_pob);

    printf("\nPozostalo %d wolnych kufli\n", kufel);
    if (kufel < 0) printf("Blad kufel zostal pobrany pomino ich braku.\n");
    ...
    // Oddanie kufla
    pthread_mutex_lock(&mutex_k_odd);
    printf("\nKlient %d, odkladam kufel %d\n", moj_id, k_odd++ - k_pob); 
    pthread_mutex_unlock(&mutex_k_odd);
  }
